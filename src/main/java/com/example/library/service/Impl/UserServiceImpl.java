...
public class UserServiceImpl implements UserService {
    ...
    public void exportTemplateExcel(HttpServletResponse response) throws IOException {
        ...
        CellRangeAddressList addressList = new CellRangeAddressList(4, 15, 3, 3);
    ...
    public void exportToExcel(UserListRequest request, HttpServletResponse response) throws IOException {
        ...
        for (UserResponse u : userResponses) {
            ...
            Cell cell0 = row.createCell(0);
            cell0.setCellValue(u.getEmail());
            cell0.setCellStyle(dataStyle);
            Cell cell1 = row.createCell(1);
            cell1.setCellValue(u.getUsername());
            cell1.setCellStyle(dataStyle);
            Cell cell2 = row.createCell(2);
            cell2.setCellValue(u.getIsActive() == 1 ? "Hoạt động" : "Không hoạt động");
            cell2.setCellStyle(dataStyle);
            Cell cell3 = row.createCell(3);
            cell3.setCellValue(u.getRoles()
                    .stream()
                    .map(RoleResponseNoPermission::getName)
    ...
    public void importFromExcel(MultipartFile file) {
        try (InputStream is = file.getInputStream()) {
            ...
            for(int i = 4; i <= sheet.getLastRowNum(); i++) {
                ...
                request.setEmail(UtilsExcel.getCellValue(row.getCell(0)));
                request.setUsername(UtilsExcel.getCellValue(row.getCell(1)));
                request.setPassword(UtilsExcel.getCellValue(row.getCell(2)));
                String isActiveStr = UtilsExcel.getCellValue(row.getCell(3));
                Integer isActive = isActiveStr.isEmpty() ? null : isActiveStr.trim().equals("Hoạt động") ? 1 : 0;
                request.setIsActive(isActive);
                String roleStr = UtilsExcel.getCellValue(row.getCell(4));
